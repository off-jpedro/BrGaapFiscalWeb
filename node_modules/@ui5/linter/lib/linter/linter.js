...
export async function lintProject({ rootDir, filePatterns, ignorePatterns, coverage, details, configPath, ui5Config, noConfig, }, sharedLanguageService) {
    ...
    res.forEach((result) => {
        result.filePath = transformVirtualPathToFilePath(result.filePath, relFsBasePath, virBasePath, relFsBasePathTest, virBasePathTest);
    });
...
export async function lintFile({ rootDir, filePatterns, ignorePatterns, namespace, coverage, details, configPath, noConfig, }, sharedLanguageService) {
    let config = {};
    if (noConfig !== true) {
        const configMngr = new ConfigManager(rootDir, configPath);
        config = await configMngr.getConfiguration();
 ...
 */
function transformVirtualPathToFilePath(virtualPath, srcFsBasePath, srcVirBasePath, testFsBasePath, testVirBasePath) {
...
export function resolveReader({ patterns, resourceReader, inverseResult = false, patternsMatch, relFsBasePath, virBasePath, relFsBasePathTest, virBasePathTest, }) {
    if (!patterns.length) {
    ...
    return createFilterReader({
        ...
        callback: (resource) => {
            ...
            const resPath = transformVirtualPathToFilePath(resource.getPath(), relFsBasePath, virBasePath, relFsBasePathTest, virBasePathTest);
