...
export default class TypeChecker {
    ...
    #workspace;
    #filePathsWorkspace;
    constructor({ workspace, filePathsWorkspace, context }, sharedLanguageService) {
    ...
    async lint() {
        ...
        for (const sourceFile of program.getSourceFiles()) {
            if (!sourceFile.isDeclarationFile && pathsToLint.includes(sourceFile.fileName)) {
                ...
                const linter = new SourceFileLinter(this.#context, sourceFile.fileName, sourceFile, sourceMaps, checker, reportCoverage, messageDetails, apiExtract, this.#filePathsWorkspace, this.#workspace, manifestContent);
                await linter.lint();
                linterDone();
            }
        }
        ...
        if (virtualXMLResources.length > 0) {
            ...
            for (const sourceFile of program.getSourceFiles()) {
                if (!sourceFile.isDeclarationFile && /\.inline-[0-9]+\.(view|fragment)\.js/.exec(sourceFile.fileName)) {
                    const linterDone = taskStart("Type-check resource", sourceFile.fileName, true);
                    const linter = new SourceFileLinter(this.#context, sourceFile.fileName, sourceFile, sourceMaps, checker, reportCoverage, messageDetails, apiExtract, this.#filePathsWorkspace, this.#workspace);
                    await linter.lint();
                    linterDone();
                }
            }
