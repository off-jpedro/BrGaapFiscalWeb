...
import { getPropertyNameText, getSymbolForPropertyInConstructSignatures, getPropertyAssignmentInObjectLiteralExpression, getPropertyAssignmentsInObjectLiteralExpression, findClassMember, isClassMethod, findAmbientModuleByDeclarationName, } from "./utils.js";
...
export default class SourceFileLinter {
    ...
    #metadata;
    #xmlContents;
    constructor(context, resourcePath, sourceFile, sourceMaps, checker, reportCoverage = false, messageDetails = false, apiExtract, filePathsWorkspace, workspace, manifestContent) {
    ...
    analyzeNewExpression(node) {
        ...
        else if (nodeType.symbol.declarations?.some((declaration) => this.isUi5ClassDeclaration(declaration, "sap/ui/base/ManagedObject"))) {
    ...
    analyzeCallExpression(node) {
        ...
        if (exprType.symbol && moduleDeclaration) {
            ...
            if (symbolName === "init" && moduleName === "sap/ui/core/Lib") {
                ...
                nodeType.symbol?.declarations?.some((declaration) => this.isUi5ClassDeclaration(declaration, "sap/ui/base/ManagedObject"))) {
                this.#analyzeNewAndApplySettings(node);
            }
    ...
    #extractXmlFromJs(node, viewType = "view") {
    ...
    */
    ...
    analyzeImportDeclaration(importDeclarationNode) {
        ...
        if (this.isSymbolOfPseudoModuleType(symbol)) {
            ...
            if (isEnum) {
                this.#reporter.addMessage(MESSAGE.NO_DIRECT_ENUM_ACCESS, { moduleName }, moduleSpecifierNode);
            }
            else { // Data Type
                this.#reporter.addMessage(MESSAGE.NO_DIRECT_DATATYPE_ACCESS, { moduleName }, moduleSpecifierNode);
            }
        }
    }
